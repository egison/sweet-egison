...
import Language.Egison.Parser.Pattern qualified as Pat
  ( parseNonGreedy,
  )
import Language.Egison.Parser.Pattern.Mode.Haskell.TH
  ( ParseMode (..),
  )
import Language.Egison.Syntax.Pattern as Pat
  ( Expr (..),
  )
import Language.Haskell.Exts.Extension
  ( Extension (EnableExtension),
  )
import Language.Haskell.Exts.Extension as Exts
  ( KnownExtension,
  )
...
parsePatternExpr haskellMode content = case Pat.parseNonGreedy mode content of
  ...
  Right x -> pure x
  where
    mode = ParseMode {haskellMode, fixities = Just listFixities}
    ...
    go (Pat.Pattern cName [p]) mName tName body = do
      ...
      pure
        $ let_
          (mNameToVar p mName')
          ( AppE
              (AppE (VarE (mkName (show cName ++ "M"))) (VarE mName))
              (VarE tName)
          )
        $ AppE
          (VarE 'fromList)
