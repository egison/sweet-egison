...
comb2 :: Int -> [(Int, Int)]
comb2 n =
  matchAll dfs [1 .. n] (List Something) [[mc| _ ++ $x : _ ++ $y : _ -> (x, y) |]]

comb2Native :: Int -> [(Int, Int)]
comb2Native n = [ (x, y) | (x:ts) <- tails [1..n], y <- ts ]

comb2Backtracking :: Int -> [(Int, Int)]
comb2Backtracking n = toList $ dfs [1..n] >>= fromList . tails >>= (\xs' -> fromList (maybeToList (uncons xs'))) >>= \(x, ys) -> fromList (tails ys) >>= (\ys' -> fromList (maybeToList (uncons ys))) >>= \(y, _) -> pure (x, y)

comb2Backtracking' :: Int -> [(Int, Int)]
comb2Backtracking' n = toList $ dfs (List Something, [1..n]) >>=
    \ (mat_a5g3, tgt_a5g4)
...
comb2' n =
  matchAll dfs [1 .. n] (List Something) [\ (mat_a5g3, tgt_a5g4)
      -> let (m_a5g5, m_a5g6) = (joinM mat_a5g3) tgt_a5g4
         in
           ((fromList (((join (WC, GP)) mat_a5g3) tgt_a5g4))
              >>=
...
comb2'' n =
  matchAllSingle dfs [1 .. n] (List Something) (\ (mat_a5g3, tgt_a5g4)
      -> let (m_a5g5, m_a5g6) = (joinM mat_a5g3) tgt_a5g4
         in
           ((fromList (((join (WC, GP)) mat_a5g3) tgt_a5g4))
              >>=
